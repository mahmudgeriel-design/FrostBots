...
public final class Main extends JavaPlugin {
    ...
    public void onEnable() {
        ...
        // Таймер заходов и выходов ботов (проверка каждые 2 минуты)
        new BukkitRunnable() {
            @Override
            public void run() {
                handleBotLifecycle();
        ...
        // Таймер для общения в чате с БОЛЬШИМ КД (проверка каждые 5 минут)
        new BukkitRunnable() {
            @Override
            public void run() {
                handleBotChat();
            }
        }.runTaskTimer(this, 600L, 6000L); // 6000 тиков = 5 минут
        
        getLogger().info("Плагин на фейк-онлайн с большим КД успешно запущен!");
    }
    ...
    private void handleBotLifecycle() {
        ...
        if (random.nextDouble() > 0.3) return; // Шанс всего 30%, что онлайн изменится
    ...
    private void handleBotChat() {
        ...
        // Шанс всего 25% при проверке каждые 5 минут. 
        // Это значит, что боты будут писать в среднем раз в 10-20 минут!
